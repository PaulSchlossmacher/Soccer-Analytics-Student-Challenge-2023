...
```{r}
t.test(x=M_FoulsPerMin$FoulsPerMin, y=W_FoulsPerMin$FoulsPerMin,
...
result_df <- data %>%
  filter(match_id %in% foul_rows$match_id &
           index %in% (foul_rows$index + 1):(foul_rows$index + 5))
...
```{r}
# Need these columns
#"index", "timestamp", "type.name", "play_pattern.name", "pass.type.id", "shot.type.id", "match_id"
...
legend("topright", legend = c("Men", "Women"), col = c("red", "green"), lty = 1, lwd = 2)
